...
经过几个月的痛定思痛，秉持着打造一门有其独特亮点的现代函数式语言的观点，Sena 决定放弃引入完整的依赖类型。
...
此处的 `*` 和 `* -> *` 即分别为 `Kind` 中的 `Star`（相当于 `Type`） 和 `Arrow`。当然 Haskell 中的 `Kind` 还包括 `Constraint` 乃至 `DataKinds` 扩展可以提升类型为 `Kind`，不过这就不在讨论范围内了。类型宇宙则提供了一个更纯粹统一的视角：
...
无限地累积下去就会变成一个垂直上升的结构，Lean4 中即是如此，而 Idris2 现阶段未实现类型宇宙，只能 `Type : Type`，这意味着可以直接构造出罗素悖论。实现依赖类型本身就很复杂，而实现依赖类型也自然会带来类型宇宙，有了类型宇宙也必须一并引入宇宙多态，如此工程量是极不划算的。为方便后文清楚表达，可以不严谨地认为 `Kind` 相当于类型宇宙中的 `Type 1`。

做这个项目的初衷一方面是一等类型确实有趣，当然依赖类型也很有趣，但对于工业导向还是太无必要了，也无需要进行定理证明的硬性需求。另一方面是哀于 TypeScript 它有着极强类型编程能力，可类型层面和值层面是完全隔离的两个世界和两套语法，实在遗憾。Zig 的编译时计算类型的特性也很不错，至少语法统一了，但碍于 Zig 的类型系统比 TypeScript 弱太多无法玩出花样，其次运行时不可用，令人遗憾。对比下来，Sena 的定位及目标也就明确了。

总的来说，一等类型意味着类型（`Type` 内的元素）具有一等性，但种类（`Kind` 或 `Type 1` 内的元素）不具有一等性，如有这样的行为：
...
通俗地说，`Type`、`Type -> Int`、`Type -> Type` 等只要出现了 `Type` 的就无法作为值表达式传递（类型宇宙视角下这些都属于 `Type 1`），但若是 `Int`或 `Int -> Int` 等则可以。换而言之，类型可以传递，类型的类型不可以传递，类型的类型之上（Type 1 以上）不存在。虽然不支持依赖类型（即类型依赖于值），但对于类型算子允许在编译时做一些静态字面量值计算，如：
...
这看着会像 Scala3 的 `TypeLambda`，因为其也支持在类型层面做 `match`，但实质截然不同。首先 Scala3 中的 `0`、`"hi"` 等是字面量类型而非值，`match` 也只是其类型层面的特殊语法。而 Sena 中的这一切均来自一等类型，与普通的值操作别无二致，只是说在参与编译时的类型计算里一切都必须纯且可停机（这将涉及后续设计细节了），在 Idris2 等语言亦是如此。
