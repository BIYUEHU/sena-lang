  ...
  const curried =
    ...
    (...rest) => {
      if (rest.length === 0 || expectedLength === 0) {
        throw new MihamaError(
          `Arguments cannot be empty for function ${f.name}`
        );
      }
...
const [MihamaKind, MihamaString, MihamaInt, MihamaBool, MihamaChar] = [
  "Kind",
  "String",
  "Int",
  "Bool",
  "Char",
].map(createMihamaType);
...
const Cons = mihamaCurry((x, xs) =>
  createMihamaValue("Cons", [createMihamaValue("Cons", [x, xs])])
);
...
const fibonacii = mihamaCurry((x) =>
  x === 1 ? 1 : x === 2 ? 1 : fibonacii(x - 1) + fibonacii(x - 2)
);
...
const toNumber = mihamaCurry((x) =>
  x.value_tag === "S" && x.value[0] === Z
    ? 1
    : x.value_tag === "S"
    ? toNumber(x.value[0]) + 1
    : x === Z
    ? 0
    : (() => {
        throw new MihamaError(
          "Match pattern not exhaustive at function toNumber"
        );
      })()
);
...
const bar = ((x) => ((y) => (x + y) ^ ((2 * 2) / 5 - (5 % 1)))(30))(20);
